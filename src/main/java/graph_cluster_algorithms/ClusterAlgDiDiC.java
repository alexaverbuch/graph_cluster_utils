...
public class ClusterAlgDiDiC {
  ...
  public void do_DiDiC(String databaseDir, int maxTimeSteps, int clusterCount,
    InitType clusterInitType, AllocType allocType, Supervisor supervisor) {
    ...
    switch (clusterInitType) {
      ...
      case BALANCED:
        ...
        break;
      case DEFAULT:
        // Graph is already coloured. Do nothing here.
        break;
    }
    ...
    for (int timeStep = 0; timeStep < maxTimeSteps; timeStep++) {
      ...
      System.out.printf("DiDiC Complete - Time Taken: %d(m):%d(s):%d(ms)%n", m,
        s, ms);
      ...
      if (supervisor.is_periodic_snapshot(timeStep)) {
        ...
        supervisor.do_periodic_snapshot(timeStep, clusterCount, databaseDir);
  ...
  private void update_cluster_allocation(int timeStep, AllocType allocType) {
    ...
    try {
      ...
      for (Entry<String, ArrayList<Double>> wC : w.entrySet()) {
        ...
        Node v = transIndexService.getNodes("name", vName).iterator().next();
        ...
        switch (allocType) {
          ...
          case OPT:
            vNewColor = allocate_cluster_intdeg(v, wC.getValue(), timeStep);
            break;
  ...
  private int allocate_cluster_intdeg(Node v, ArrayList<Double> wC, int timeStep) {
